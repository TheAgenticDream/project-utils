...
class DBFunctions:
    ...
    def insert(
        self, table: str, data: dict[str, Any], returning: str = None
    ) -> Any | None:
        """
        ...
        return self.db.execute(
            execution_string=insert_string, commit=True, fetch_one=bool(returning)
        )
    ...
    def update(
        ...
        return self.db.execute(
            execution_string=update_string, commit=True, fetch_one=bool(returning)
        )
    ...
    def select(
        ...
        return self.db.execute(
            execution_string=select_string, fetch_all=fetch_all, fetch_one=not fetch_all
        )

    def delete(
        self, table: str, where: dict[str, Any], returning: str = None
    ) -> Any | None:
        """
        ...
        return self.db.execute(
            execution_string=delete_string, commit=True, fetch_one=bool(returning)
        )
    ...
    def sanitize_json(data: Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            return data.replace(
                "'", "$"
            )  # .replace('"', "$") # Do not replace " with $ to avoid DB errors at publish_work_package step.
        else:
            return data
